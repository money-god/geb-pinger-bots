...
import { contracts } from 'mcgeb.js'
...
export class TaxCollectorPinger {
  ...
  public async ping() {
    ...
    const numCollaterals = (await this.taxCollector.collateralListLength())
    const tx = this.taxCollector.taxMany(0, numCollaterals - 1)
    const hash = await this.transactor.ethSend(tx, true, TAX_COLLECTOR__TAX_SINGLE_GAS * numCollaterals)
